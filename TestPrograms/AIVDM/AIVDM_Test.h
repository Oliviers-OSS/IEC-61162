...
#ifndef AIVDMTESTS_H_
#define AIVDMTESTS_H_
...
class AIVDM_Tests: public CxxTest::TestSuite {
    IEC61162::AIVDMMessage msg;
    bool callbackCalled;
...
public:
    ...
    AIVDM_Tests():msg(this),callbackCalled(false),currentMsg(0),lastHandlerTestResult(EXIT_SUCCESS) {
    ...
    void test_AISVDMMsg(void) {
        register unsigned int nbMsg = sizeof(VDMdata) / sizeof(AISMsg1_Data);
        currentMsg = VDMdata;
        while(nbMsg > 0) {
            nbMsg--;
            ...
            TS_ASSERT_EQUALS(lastHandlerTestResult,EXIT_SUCCESS);
            TS_ASSERT_EQUALS(callbackCalled,true);
            currentMsg++;
        }
    }

    void test_AISMultiSentenceVDMMsg() {
        ...
        TS_ASSERT_EQUALS(error,EXIT_SUCCESS);
        TS_ASSERT_EQUALS(lastHandlerTestResult,EXIT_SUCCESS);
        TS_ASSERT_EQUALS(callbackCalled,false);
        callbackCalled = false;
    ...
    void test_AISMultiSentenceVDMMsgReverseOrder() {
