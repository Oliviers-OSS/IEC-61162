...
void msgHandler(IEC61162::AISMsg6 msg,void *param) {
    AISMsg6_Tests *caller = static_cast<AISMsg6_Tests *>(param);
    if (caller) {
        // check the decoded message's content against the references data
        const struct AISMsg6_Data *test_data = caller->currentMsg;
        char buffer[255];
        ...
        TS_ASSERT_EQUALS(msg->get_retransmitFlag(),test_data->retransmitFlag);
    } else {
