...
#ifndef _AIS_MESSAGE_2_TEST_H_
#define _AIS_MESSAGE_2_TEST_H_
...
void msgHandler(IEC61162::AISMsg2 msg,void *param);

class AISMsg2_Tests : public CxxTest::TestSuite
{
    IEC61162::AISMessage msg;
    const AISMsg2_Data *currentMsg;
    int lastHandlerTestResult;
public:

    friend void msgHandler(IEC61162::AISMsg2 msg,void *param);

    AISMsg2_Tests()
        :msg(this),currentMsg(0),lastHandlerTestResult(EXIT_SUCCESS) {
        msg.message2.connect(sigc::ptr_fun(msgHandler));
    }
    ...
    void test_AISMsg(void) {
        register unsigned int nbMsg = sizeof(data) / sizeof(AISMsg2_Data);
        TS_ASSERT(nbMsg >0);
        currentMsg = data;
        while(nbMsg > 0) {
            nbMsg--;
