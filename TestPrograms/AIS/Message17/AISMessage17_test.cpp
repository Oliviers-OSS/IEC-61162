...
void msgHandler(IEC61162::AISMsg17 msg,void *param) {
    AISMsg17_Tests *caller = static_cast<AISMsg17_Tests *>(param);
    if (caller) {
        // check the decoded message's content against the references data
        const struct AISMsg17_Data *test_data = caller->currentMsg;
